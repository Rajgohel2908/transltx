  ...
  try {
    ...
    const request = {
      ...
      customer_details: {
        ...
        customer_phone: user.phone || "9999999999", // Cashfree ko phone chahiye
        customer_email: user.email,
        customer_name: user.name,
      },
      order_meta: {
        // Cashfree ko {order_id} placeholder chahiye
        return_url: `${process.env.CLIENT_URL}/orders?order_id={order_id}`,
      },
      order_note: `Booking for: ${itemName}`,
    };
    
    // API version 2023-08-01 (latest use kar)
    const response = await Cashfree.PG.Orders.Create(request, {
       "x-api-version": "2023-08-01"
    });
    
    // Client ko 'payment_session_id' chahiye, 'data' nahi
