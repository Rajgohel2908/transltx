...
// CashfreePG.Cashfree.XClientId = process.env.CASHFREE_APP_ID;
// CashfreePG.Cashfree.XClientSecret = process.env.CASHFREE_SECRET_KEY;
// // Use SANDBOX for development and PRODUCTION for production
// CashfreePG.Cashfree.XEnvironment = CashfreePG.Environment.SANDBOX;
...
export const createTripOrder = async (req, res) => {
  ...
  //   const response = await CashfreePG.Cashfree.PGCreateOrder("2022-09-01", request);
  ...
  // }
  res.status(503).json({ message: "Payment integration is temporarily disabled." });
};